...
public class AdminServiceImpl implements AdminService{
    ...
    public TaskDTO createTask(TaskDTO taskDTO) {
        ...
        if(optionalUser.isPresent()){
            ...
            task.setTaskStatus(TaskStatus.INPROGESS);
    ...
    public List<TaskDTO> searchTaskByTitle(String title) {
         return taskRepository.findAllByTitleContaining(title).stream().sorted(Comparator.comparing(Task::getDueDate).reversed()).map(Task::getTaskDTO).collect(Collectors.toList());
    }

    private TaskStatus mapStringToTaskStatus(String status){
        return switch (status) {
            case "PENDING" -> TaskStatus.PENDING;
            case "INPROGRESS" -> TaskStatus.INPROGESS;
